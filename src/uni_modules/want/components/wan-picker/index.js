...
export function setup(props, context) {
  ...
  const getChangeIndex = (values) => {
    ...
    if (state.pickerValues.length < 1) {
      return values.findIndex((item) => item > 0);
    }
    return values.findIndex((item, index) => {
  ...
  const setColumnValues = (index, options, needReset = true) => {
