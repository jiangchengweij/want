...
export const emits = ['cancel', 'confirm', 'change'] as const;
export const props = {
  ...pickerProps,
  showToolbar: {
    type: Boolean,
...
export function setup(props: Props, context: Context) {
  ...
  const emitEvent = (type: 'cancel' | 'confirm', detail) => {
    detail.values = detail.value;
    delete detail.value;
    emit(type, detail);
  };

  const parseValues = (values: AreaItem[]) => {
    const { columnsPlaceholder } = props;
  ...
  const getList = (type: string, code?: string): AreaItem[] => {
    ...
    if (code != null) {
      ...
      result = result.filter((item) => item.code.indexOf(code as string) === 0);
    }
  ...
  const getIndex = (type: string, code: string) => {
