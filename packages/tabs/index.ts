...
export function setup(
  ...
  const state = reactive({
    tabs: [] as Record<string, AnyObject>[],
    scrollLeft: 0,
    scrollable: false,
    currentIndex: 0,
    skipTransition: true,
  ...
  const trigger = (eventName: 'change' | 'disabled' | 'click', child?) => {
  ...
  const getChildData = (index: number, child?: ComponentInternalInstance) => {
  ...
  const getAvaiableTab = (direction: number) => {
